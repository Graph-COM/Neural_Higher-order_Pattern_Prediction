...
def run(args):
    ...
    if not args.one_hot_node:
        ...
        print('max_idx ', max_idx, ' min_idx ', min(new_df.u.min(), new_df.i.min()))
        rand_feat = np.zeros((max_idx + 1, feat.shape[1]))
        if 'socialevolve' in data_name or 'mathoverflow' in data_name:
...
parser.add_argument('--dataset', choices = ['wikipedia', 'reddit', 'socialevolve', 'socialevolve_1month', 'socialevolve_2weeks', 'mooc', 'sx-mathoverflow'],
